...
if os.path.exists('perfiles.json'):
    with open('perfiles.json','r') as archivo:
        ...
        lista_fotos = list(map(lambda elem : elem,datos))
        for foto in lista_fotos:
            print(f"Ruta imagen {foto['Foto']}")
else:
...
if(len(lista_fotos) > 0):
    ...
    for foto in lista_fotos:
        with open(foto['Foto'], 'rb') as file:
            img_bytes = file.read()
...
layout = [[sg.Text('UNLP-Image', size=(50, 2), font=('Times New Roman', 50), text_color='Black', justification=("c"))],
        [*imagenes],
        [sg.Button("Agregar perfil",size=(20, 2), button_color=('white', 'grey'), font=('Helvetica', 12),key='agregar_perfil'),
